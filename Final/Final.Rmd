...
$$\log(\frac{p_{ij}}{1-p_{ij}}) = \mu + GPA_{ij}\beta_1 + \text{Extracurricular}_{ij}\beta_2+ \text{Work Experience}_{ij} \beta_3+ U_i$$
$$U_i \sim N(0, \sigma^2)$$
where $Y_{ij}$ is the acceptance of applicant $i$ in application group $j$, X_{ij} is the 
```{r}
## Phase 1 -> Phase 2
