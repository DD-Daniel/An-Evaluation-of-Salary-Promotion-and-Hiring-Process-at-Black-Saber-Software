...
Our outcome of interest is whether the applicant passes phase 1, and our predictors are GPA, work experience, extracurricular. As we observe similar pattern for individual in one team, a generalized linear mixed model with logit link was used and random intercepts were estimated by the team that each applicant desire to work with.\newline

The model can be written as:
$$Y_{ij} \sim Bernoulli(p_{ij}) $$
$$\log(\frac{p_{ij}}{1-p_{ij}}) = \mu + \beta_1 GPA_{ij} + \beta_2\text{Extracurricular}_{ij}+ \beta_3\text{Work Experience}_{ij} + T_i$$
$$T_i \sim N(0, \sigma^2)$$
where $Y_{ij}$ is the acceptance of applicant $i$ in application group $j$, X_{ij} are fixed effects and $T_i$ is random effect of applicants' desired team, which follows normal distribution.\newline

The model is realized in R through the `glmer` function in the `lme4` package. Result is shown in table 1. Not surprisingly, we find that GPA and work experiences are all significant at level 0.05. 1 unit increase of GPA is associated with 
...
rownames(table) <- c("Baseline Odds", "GPA", "Extracurriculas", "Work Experience", "Gender: Prefer Not to say", "Gender: Woman")
kable(table)
```
